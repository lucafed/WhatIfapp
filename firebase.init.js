// === IMPORT da firebase.init.js + crediti ===
...
// (oppure, se vuoi, esportiamo onAuthStateChanged da firebase.init.js e lo prendi da lì)
...
(function () {
  ...
  if (form && out) {
    form.addEventListener("submit", async (e) => {
      ...
      const tags = [...form.querySelectorAll('input[name="tags"]:checked')].map(
        (c) => c.value
      );
      ...
      if (mode === "whataf") {
        prob = Math.max(5, Math.min(95, prob - (seed % 17) + 8));
      }
      ...
      const blurb =
        mode === "sliding"
          ? `In uno scenario ${
              tense === "past" ? "alternativo passato" : "plausibile futuro"
            }, è probabile che ${core}.${extra}`
...
function openCreditStore() {
  ...
  back.style.cssText =
    "position:fixed;inset:0;background:rgba(0,0,0,.55);backdrop-filter:blur(2px);z-index:9998";
  back.addEventListener("click", close);

  overlayFrame = document.createElement("iframe");
  overlayFrame.src = "/store/credit-store.html?lang=it"; // <-- percorso del tuo file store
  overlayFrame.style.cssText =
    "position:fixed;left:50%;top:50%;transform:translate(-50%,-50%);width:min(520px,92vw);height:360px;border:0;border-radius:16px;z-index:9999;box-shadow:0 20px 60px rgba(0,0,0,.6)";
