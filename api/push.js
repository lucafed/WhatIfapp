...
// NOTA: SOLO data → nessun blocco notification → tutto passa da sw.js
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // URL completo che deve aprirsi al tap
    const CLICK_LINK =
    ...
    // 🔴 IMPORTANTE: SOLO DATA, NIENTE `notification`, NIENTE `webpush.fcmOptions.link`
    const message = {
      data: {
        title,
        body,
        ...
        url: CLICK_LINK,
        click_action: CLICK_LINK // per sicurezza, usato dal sw.js
      },
      tokens
    };
